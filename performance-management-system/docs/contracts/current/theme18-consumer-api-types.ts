/**
 * 主题18开发前评审的 consumer 契约快照。
 * 这里只固化前端调用的最小类型和接口路径，作为 schema drift 与 endpoint drift 的消费侧证据，不负责真实页面实现。
 * 维护重点是只消费录用主题冻结范围内的字段与来源摘要，不把简历联系方式、面试评语或入职动作混入 API 契约。
 */
...
const hiringBaseUrl = "/admin/performance/hiring";
...
export function fetchHiringPage(data: HiringPageQuery) {
  return request({
    url: `${hiringBaseUrl}/page`,
    method: "POST",
    data,
  });
}

export function fetchHiringInfo(params: { id: number }) {
  return request({
    url: `${hiringBaseUrl}/info`,
    method: "GET",
    params,
  });
}

export function createHiring(data: HiringSaveRequest) {
  return request({
    url: `${hiringBaseUrl}/add`,
    method: "POST",
    data,
  });
}

export function updateHiringStatus(data: HiringStatusUpdateRequest) {
  return request({
    url: `${hiringBaseUrl}/updateStatus`,
    method: "POST",
    data,
  });
}

export function closeHiring(data: HiringCloseRequest) {
  return request({
    url: `${hiringBaseUrl}/close`,
    method: "POST",
    data,
  });
}
