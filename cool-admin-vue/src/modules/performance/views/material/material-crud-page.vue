<!-- 文件职责：承接物资管理一期页面的通用列表、筛选、详情与表单壳层；不负责具体资源契约、菜单接线或跨主题复用；依赖 Element Plus、权限工具与绩效列表页状态 composable；维护重点是 catalog/stock/inbound/issue 四页的查询、详情和动作入口必须保持一致。 -->
<template>
	<div v-if="canAccess" class="material-crud-page" :data-testid="resolvePageTestId()">
		<el-card shadow="never">
			<div class="material-crud-page__header">
				<div>
					<div class="material-crud-page__eyebrow">物资管理 / 一期</div>
					<h2>{{ title }}</h2>
					<p>{{ description }}</p>
				</div>
		...
		<el-card shadow="never">
			<div class="material-crud-page__toolbar">
				<div class="material-crud-page__filters">
				...
				<div class="material-crud-page__actions">
					<el-button data-testid="material-search-button" @click="search">查询</el-button>
					<el-button data-testid="material-reset-button" @click="reset()">重置</el-button>
					<template v-for="action in toolbarActions" :key="action.key">
						<el-button
							v-if="checkPerm(action.permission)"
							:type="resolveButtonType(action.type)"
							:data-testid="resolveToolbarActionTestId(action.key)"
					...
					<el-button
						...
						type="primary"
						data-testid="material-create-button"
						@click="openCreate"
					>
						{{ createLabel || '新增' }}
					</el-button>
		...
		<el-card shadow="never">
				...
				<el-table-column label="操作" fixed="right" min-width="280">
					<template #default="{ row }">
						...
						<el-button
							...
							text
							type="primary"
							:data-testid="resolveRowActionTestId('edit', row.id)"
							@click="openEdit(row)"
						>
							编辑
						</el-button>
						<el-button
							...
							text
							type="danger"
							:data-testid="resolveRowActionTestId('delete', row.id)"
							@click="removeRow(row)"
						>
							删除
						</el-button>
			...
			<div class="material-crud-page__pagination">
				<el-pagination
					background
		...
		>
					...
					<el-col v-for="field in formFields" :key="field.prop" :span="field.span || 12">
						<el-form-item :label="field.label">
							<el-input
								...
								:placeholder="field.placeholder || field.label"
								:data-testid="resolveFormFieldTestId(field.prop)"
			...
			<template #footer>
				...
				<el-button
					type="primary"
					:loading="submitting"
					data-testid="material-form-save-button"
					@click="submitForm"
				>
					保存
				</el-button>
...
function resolvePageTestId() {
	return `material-page-${sanitizeTestIdPart(props.title)}`;
}

function resolveFilterTestId(prop: string) {
	return `material-filter-${sanitizeTestIdPart(prop)}`;
}

function resolveFormFieldTestId(prop: string) {
	return `material-form-${sanitizeTestIdPart(prop)}`;
}

function resolveToolbarActionTestId(key: string) {
	return `material-toolbar-${sanitizeTestIdPart(key)}`;
}

function resolveRowActionTestId(key: string, rowId?: string | number) {
	return `material-row-${sanitizeTestIdPart(key)}-${sanitizeTestIdPart(String(rowId || 'unknown'))}`;
}
</script>
...
.material-crud-page {
	@include metadataWorkspace.metadata-workspace-shell(1160px);
