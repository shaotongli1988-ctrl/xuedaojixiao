<!-- 文件职责：承接主题11供应商台账的列表、详情、新增、编辑和删除主链；不负责资质、评级、结算中心、附件全文或外部采购系统接入；依赖 supplier service 与既有权限工具；维护重点是角色展示统一消费 access-context 事实源，敏感字段展示仍必须与冻结口径一致。 -->
<template>
...
const filterStatusOptions = computed<Array<{ label: string; value: SupplierStatus }>>(() =>
	dict.get(SUPPLIER_STATUS_DICT_KEY).value.map(item => ({
		label: item.label,
		value: item.value as SupplierStatus
	}))
);
...
const readOnlyRoleNotice = computed(
	() => `当前账号为${roleFact.value.roleLabel}，敏感字段仅展示后端返回的脱敏摘要。`
);
const supplierList = useListPage({
	createFilters: () => ({
		keyword: '',
		category: '',
	...
	fetchPage: async params =>
		performanceSupplierService.fetchPage({
			page: params.page,
			size: params.size,
	...
	onError: (error: unknown) => {
		showElementErrorFromError(error, '供应商列表加载失败');
	}
});
const rows = supplierList.rows;
const tableLoading = supplierList.loading;
const filters = supplierList.filters;
const pagination = supplierList.pager;

onMounted(async () => {
	await Promise.all([dict.refresh([SUPPLIER_STATUS_DICT_KEY]), loadAccessContext()]);
	await refresh();
});

async function refresh() {
	await supplierList.reload();
}
...
function handleSearch() {
	void supplierList.search();
}

function handleReset() {
	void supplierList.reset();
}

function changePage(page: number) {
	void supplierList.goToPage(page);
}
...
async function openDetail(row: SupplierRecord) {
	...
	try {
		...
		showElementErrorFromError(error, '供应商详情加载失败');
	}
}
...
async function submitForm() {
	...
	try {
		...
		showElementErrorFromError(error, '供应商保存失败');
	} finally {
...
async function handleDelete(row: SupplierRecord) {
	...
	const rowId = row.id;
	const confirmed = await confirmElementAction(`确认删除供应商「${row.name}」吗？`, '删除确认');
	...
	await runTrackedElementAction({
		...
		actionType: 'delete',
		request: () =>
			performanceSupplierService.removeSupplier({
				ids: [rowId]
			}),
		successMessage: '删除成功',
		errorMessage: '供应商删除失败',
		refresh
	});
}
...
function statusLabel(status?: SupplierStatus | '') {
	return dict.getLabel(SUPPLIER_STATUS_DICT_KEY, status) || status || '-';
}

function statusTagType(status?: SupplierStatus | '') {
	return dict.getMeta(SUPPLIER_STATUS_DICT_KEY, status)?.tone || 'success';
}
...
.supplier-page {
	@include managementWorkspace.management-workspace-shell(1280px);
}
</style>
