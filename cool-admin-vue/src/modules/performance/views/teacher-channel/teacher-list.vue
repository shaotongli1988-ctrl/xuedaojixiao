		...
		<div class="teacher-channel-teacher-page">
			...
			<el-drawer v-model="detailVisible" title="班主任详情" size="920px" destroy-on-close>
				<div v-loading="detailLoading" class="teacher-channel-teacher-page__drawer">
					...
					<section
						v-if="detailTeacher"
						class="teacher-channel-teacher-page__follow-section"
					>
						<div class="teacher-channel-teacher-page__section-header">
							<div>
								<h3>代理归因</h3>
								<p>
									当前归因、冲突态和历史都以后端返回结果为准，不在前端伪造生效态。
								</p>
							</div>
							<el-tag effect="plain">
						...
						<template v-else>
							<el-descriptions
								v-if="attributionInfo?.currentAttribution"
								:column="2"
								border
							>
				...
				<div class="teacher-channel-teacher-page__drawer">
					...
					<el-table :data="agentRows" border v-loading="agentLoading">
						...
						<el-table-column label="操作" fixed="right" min-width="240">
							<template #default="{ row }">
								<el-button
									v-if="showAgentUpdateButton"
									text
									@click="openAgentForm(row)"
								>
									编辑
								</el-button>
								<el-button
					...
					<el-table :data="relationRows" border v-loading="relationLoading">
						...
						<el-table-column label="操作" fixed="right" min-width="160">
							<template #default="{ row }">
								<el-button
									v-if="showAgentRelationUpdateButton"
									text
									@click="openRelationForm(row)"
								>
									编辑
								</el-button>
								<el-button
				...
				<div v-if="conflictDetail" class="teacher-channel-teacher-page__drawer">
					...
					>
						...
						<el-form-item label="处理说明">
							<el-input
								v-model="conflictResolveForm.resolutionRemark"
								maxlength="200"
								show-word-limit
							/>
						</el-form-item>
			...
			>
				<el-form :model="relationForm" label-width="100px">
					<el-form-item label="父级代理">
						<el-select
							v-model="relationParentAgentModel"
							filterable
							style="width: 100%"
						>
							<el-option
								v-for="item in relationAgentOptions"
								:key="item.id"
								:label="item.name"
								:value="Number(item.id || 0)"
							/>
				...
				<template #footer>
					...
					<el-button
						type="primary"
						:loading="relationFormLoading"
						@click="submitRelationForm"
					>
						保存
					</el-button>
				</template>
...
onMounted(async () => {
	await Promise.all([dict.refresh([TEACHER_COOPERATION_STATUS_DICT_KEY]), loadAccessContext()]);
	await loadUsers();
...
async function submitRelationForm() {
	...
	try {
		const payload = {
			...
			effectiveTime: normalizeOptionalText(relationForm.effectiveTime as string | undefined),
...
async function submitConflictResolve(resolution: 'resolved' | 'cancelled') {
	...
	try {
		await performanceTeacherAttributionConflictService.resolveConflict({
			...
			resolution,
			agentId:
				resolution === 'resolved' ? (conflictResolveForm.winnerAgentId ?? null) : undefined,
