...
<template>
	<AssetCrudPage
		...
		page-permission="performance:assetMaintenance:page"
		add-permission="performance:assetMaintenance:add"
		update-permission="performance:assetMaintenance:update"
		delete-permission="performance:assetMaintenance:delete"
		:columns="columns"
		:filters="filters"
...
import { createEmptyAssetMaintenance } from '../../types';
...
const rowActions = [
	{
		key: 'complete',
		label: '完成',
		permission: 'performance:assetMaintenance:complete',
		visible: (row: any) => ['scheduled', 'inProgress'].includes(row.status),
		confirmText: () => '确认完成该维护记录吗？',
		handler: (row: any) => performanceAssetMaintenanceService.completeMaintenance({ id: row.id }),
		successMessage: '维护已完成'
	},
	{
		key: 'cancel',
		label: '取消',
		permission: 'performance:assetMaintenance:cancel',
		type: 'danger',
		visible: (row: any) => ['scheduled', 'inProgress'].includes(row.status),
		confirmText: () => '确认取消该维护记录吗？',
		handler: (row: any) => performanceAssetMaintenanceService.cancelMaintenance({ id: row.id }),
...
function notifyLookupError(error: any) {
	ElMessage.warning(error?.message || '维护资产选项加载失败');
}
</script>
