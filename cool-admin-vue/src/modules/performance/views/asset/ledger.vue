...
<template>
	<AssetCrudPage
		...
		page-permission="performance:assetInfo:page"
		info-permission="performance:assetInfo:info"
		add-permission="performance:assetInfo:add"
		update-permission="performance:assetInfo:update"
		delete-permission="performance:assetInfo:delete"
		:columns="columns"
...
import { createEmptyAssetInfo } from '../../types';
import {
...
const rowActions = [
	{
		...
		label: '转可用',
		permission: 'performance:assetInfo:updateStatus',
		visible: (row: any) => row.assetStatus !== 'available',
...
function notifyLookupError(error: any) {
	ElMessage.warning(error?.message || '资产基础选项加载失败');
}
</script>
