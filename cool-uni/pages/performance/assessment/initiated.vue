<!-- 文件职责：承接 cool-uni 已发起考核列表；不负责新建考核或审批动作；维护重点是只按 initiated 模式读取，并复用既有评估详情页面。 -->
<template>
...
const allowed = computed(() => user.canAccessRoute("/pages/performance/assessment/initiated"));
...
async function load() {
	...
	try {
		...
		state.error = error?.message || "已发起考核加载失败";
	} finally {
		state.loading = false;
	}
}
