<!-- 文件职责：承接 cool-uni 能力地图移动列表；不负责能力项详情、画像摘要或桌面端维护动作；维护重点是只消费能力模型摘要字段。 -->
<template>
	<cl-page background-color="#f4f6fb">
		<scroll-view class="capability-page" scroll-y>
			...
			<page-state
				v-if="!allowed"
				title="无权限访问该页面"
				description="当前账号没有能力地图移动端入口。"
				action-text="返回工作台"
				@action="goHome"
			/>

			<view v-else class="capability-page__body">
					...
					<view v-for="item in rows" :key="item.id" class="capability-card">
						<view class="capability-card__top">
							...
							<status-pill
								...
								:tone="capabilityStatusTone(item.status)"
							/>
						</view>

						<view class="capability-card__grid">
							<text>能力项数量：{{ item.itemCount ?? 0 }}</text>
...
const allowed = computed(() => user.canAccessRoute("/pages/performance/capability/index"));
const statusTabs = computed(() => [
	{ label: "全部", value: "all" },
	...dict.get(CAPABILITY_STATUS_DICT_KEY).map((item) => ({
		label: item.label,
		value: String(item.value),
	})),
]);
const capabilityList = useListPage<CapabilityModelRecord>({
	canLoad: () => allowed.value,
	fetchPage: async () => {
		await dict.refresh([CAPABILITY_STATUS_DICT_KEY]);
		return performanceCapabilityService.fetchPage({
			page: 1,
			size: 20,
	...
	resolveError: (error) => (error as any)?.message || "能力模型加载失败",
});
const rows = capabilityList.rows;
const loading = capabilityList.loading;
const error = capabilityList.error;
const load = capabilityList.reload;
...
function capabilityStatusLabel(value?: string | null) {
	return dict.getLabel(CAPABILITY_STATUS_DICT_KEY, value) || value || "未知";
}

function capabilityStatusTone(value?: string | null): "info" | "warning" | "success" | "error" {
	const tone = dict.getMeta(CAPABILITY_STATUS_DICT_KEY, value)?.tone;
