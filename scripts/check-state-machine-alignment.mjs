#!/usr/bin/env node
/**
 * 负责统一调用仓库内状态机对齐守卫，供本地 gate、CI 和发布门禁复用。
 * 不负责定义状态流规则本身，也不替代模块级状态回归测试。
 * 依赖同目录下的 `state_machine_guard.py` 与本机 `python3`。
...
const command = [
	path.join(scriptDir, 'state_machine_guard.py'),
	'--phase',
	args.phase,
	'--cwd',
	repoRoot
...
if (result.error) {
	console.error(`[state-machine guard] failed to execute: ${result.error.message}`);
	process.exit(1);
}
