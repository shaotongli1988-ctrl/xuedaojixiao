#!/usr/bin/env node
/**
 * 负责统一调用仓库内实现层收敛守卫，供本地 gate、CI 和发布门禁复用。
 * 不负责定义页面抽象策略本身，也不替代真实前端回归验证。
 * 依赖同目录下的 `component_reuse_guard.py` 与本机 `python3`。
 * 维护重点：phase、force、all 和报告输出语义要稳定，避免本地/CI 判断分叉。
 */
...
const command = [
	path.join(scriptDir, 'component_reuse_guard.py'),
	'--phase',
	args.phase,
	'--cwd',
	repoRoot
...
if (result.error) {
	console.error(`[component-reuse guard] failed to execute: ${result.error.message}`);
	process.exit(1);
}
