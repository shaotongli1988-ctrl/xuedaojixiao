/**
 * Ensures required local performance tables exist before local backend boot or smoke.
 * This file only patches known missing local-development schema artifacts and does not replace formal migrations.
 * Maintenance pitfall: keep the local DDL aligned with the corresponding TypeORM entity definitions when fields or indexes change.
 */
...
async function ensureSuggestionTable(connection) {
  const [rows] = await connection.query(
    "SHOW TABLES LIKE 'performance_suggestion';"
  );
  ...
  console.log(
    existed
      ? '[schema] performance_suggestion already exists'
      : '[schema] created performance_suggestion'
  );
}
...
async function main() {
  ...
  try {
    await ensureSuggestionTable(connection);
  } finally {
